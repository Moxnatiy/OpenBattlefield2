...
| a soldier sinking into the ground | **done**: other soldiers are carried by the soldier physics the way `predict` seeds it (`carryRemoteSoldier`, Linux 0x5dc314 / 0x6f1de0 / 0x6f2310, docs/functions/network-events.md) and the frame blends the last two ticks. Frames with a soldier's feet more than 5 cm under the terrain, one 2400-frame run: ~8500 predicted, ~130 drawn. What is left: the forces the physics manager accumulates into the node (+0x38, +0xac) are not read, and the carry costs frame rate — 115 to 85 frames a second with a dozen soldiers, the collision queries per tick | no drawn soldier's feet under the terrain, at the old frame rate |
