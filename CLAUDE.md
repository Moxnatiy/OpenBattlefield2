...
| the spawn screen on an idle round | with a live round it rebuilds fifteen times and fills (157 pieces) and DONE works; on a round where nothing moves it rebuilds **once**, empty, and the player cannot spawn at all | the spawn screen fills on a server whose bots have not spawned yet |
...
| ping | ours and the original's differ by 5–6× on the same machine. Measured from the inside: an action is answered in 172 ms on average (at most 272), and the server sends about ten packets a second whatever the connection type says. Whether the original waits as long on the same server is not measured | the difference is within noise, and `--group` then reports a soldier drawn interpolated |
| the spawn screen on an idle round | it builds empty (pieces 0) and the player cannot press DONE at all. Until it is fixed a client never spawns, never sends, and the server starves its ghost stream — which is what made other players jump ten metres between frames (docs/functions/network-events.md). `--group <n>` spawns without the screen meanwhile | the screen builds its pieces on the same server `--group` spawns on |
