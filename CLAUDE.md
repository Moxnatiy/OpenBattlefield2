...
6. **Жодних підігнаних чисел.** Це порт один в один, а не «схоже».
   ...
   * прямий вимір знімка кадру оригіналу (`Ctrl+Shift+D`, конкретний
     виклик малювання).

   Підбирати число так, щоб «стало схоже», — заборонено, навіть коли
   результат візуально збігається. Якщо джерела немає, лишаємо як є,
   пишемо в коментарі «не виміряно» і кажемо про це вголос, а не ставимо
   правдоподібне значення. Непрямий висновок (вивести розмір значка з
   положення підпису) — це теж підгонка: він спирається на здогад про
   зв'язок, якого ми не перевірили.

7. **Поведінку теж реверсимо, не лише числа.** Мало знати, *яке* значення
   має змінна — треба знати, **хто і коли** його ставить в оригіналі.
   Якщо ми вмикаємо прапорець інтерфейсу «щоб з'явилося те, що треба» —
   це така сама вигадка, як підігнане число, навіть коли на екрані все
   правильно.

   Тому кожне присвоєння змінної HUD має посилатися на своє джерело:
   стан із `docs/functions/hud-states.md`, консольну команду з
   `docs/functions/hud-commands.md` або конкретне місце в бінарі. Те, що
   ми ставимо без такого джерела, позначаємо в коментарі «джерело не
   знайдене» — і воно лишається в боргу, а не вдає реверс.

8. **Clean-room.** Код у `src/` пишемо за конспектами поведінки, не копіюємо
