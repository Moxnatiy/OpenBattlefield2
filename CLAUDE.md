...
| the velocity request after a correction | **done**: it travels in the state as the node's friction (0x200, `(surface speed − velocity) × 30`), with the acceleration (0x100), the local linear speed (0x40000) and the ground flag (bit D) — `SoldierNetworkable::updateStateMask` Linux 0x5db130 / `setNetUpdate` 0x5dd769, docs/functions/soldier-physics.md. The replay takes them. What the state does not carry (the surface speed a jump leaves, the jump's timers) comes from our own record of that tick | — |
...
| the soldier in the air | **done**: the server's tick order (input, node, collision — `GameServer::simulateFrame` 0x45af70), the node whole with its drag (0x6f2130), the jump (0x54fb7a), steering in the air (0x54f0a5), the friction (0x6f3390) and the terrain push-out on the engine's triangles (0x6f5e30, 0x6fbc00) — `tickSoldier`. Contacts with objects are the engine's too (`checkSoldierVsMesh` 0x6f44a0 over `getDistanceToSphere` 0x71a500: faces, edges, corners), on the lod the engine's table answers (a vehicle lod stands in for a missing soldier lod, `CollisionMeshTemplate::load` 0x71f600), with a bundle's child parts (0x574760) and the objects the server spawned (`CollisionLibrary`, `syncMovableCollision`). On the live server, a run strafing into a wall, jumping at it and running along it: 343 of 344 states within 1 mm of our prediction; a 5000-frame run of strafes, jumps and sprints on Strike at Karkand: 5 states over 1 cm of 986, all of them the action buffer's. What is left: the states where the server moves the soldier by two ticks or none for one action (the action buffer rows above), the spawn's first state (0.19 m), the terrain's material map (`HeightmapPrimary.mat`, `Grass` stands in) and the collision faces' materials, the water half of 0x6f5e30, `solveImpulse`'s re-test loop (`coll-soldier-collision-test-count`), moving objects' own speed at a contact, and soldiers against soldiers (`checkSoldierVsSoldier` 0x6f4150) | a run through a building draws no correction |
...
| a soldier sinking into the ground | **done**: other soldiers are carried by the soldier physics the way `predict` seeds it (`carryRemoteSoldier`, Linux 0x5dc314 / 0x6f1de0 / 0x6f2310, docs/functions/network-events.md) and the frame blends the last two ticks. Frames with a soldier's feet more than 5 cm under the terrain, one 2400-frame run: ~8500 predicted, ~130 drawn. What is left: the forces the physics manager accumulates into the node (+0x38, +0xac) are not read. It costs no frame rate: 0.3% of the main thread in `sample`, and an A/B of the same run with `--draw-predicted` gave 79.2 and 82.1 frames a second carried against 82.5 and 80.3 predicted | no drawn soldier's feet under the terrain |
...
| a vehicle's rotation | the quaternion in a simple object's update is not read; moving vehicles keep the spawner's rotation, and so does their collision (the create event's rotation, whose sign against the level's is seen only on zero rotations) | a turning jeep turns on screen |
...
| a sprinter's clip | **the original plays it; we do not** (watched by the maintainer). Read so far, and every piece agrees with itself: `Soldier::updateAnimationSystems` (Linux 0x5545e0) hands a soldier who cannot fire `getSoldierSpeed(4)` = 7.0 straight ahead, otherwise the planar length of `getAbsolutePositionalSpeed` — the node's displacement per update × 30 (`postFrameUpdate`, 0x6ddca0); `stand_sprint` is a `ForwardTrigger` on `3p_sprint` 5.5–6.3; the client's `isWithinRange` (0x7fe720) is `a <= value <= b`. Ruled out by measurement: that displacement speed on our carried soldiers — median 6.64 over 1400 sprinting frames, 27 inside the range. What the original's animation system actually holds for a sprinting remote soldier is not measured, and statics alone will not settle it | a read of the running original's animation system (+0x20 speed, +0x1c forward) for a sprinting bot, and then that bot plays the sprint clip here |
...
**Physics.** Our own soldier's tick is the engine's (the protocol table's
"the soldier in the air"). The hosted game's local server still runs our older
blend (`moveSoldier`), not the engine's tick. Measure: `--hosted` moves the
soldier through `tickSoldier`.
