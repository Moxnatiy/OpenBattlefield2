...
| soldiers as models | other soldiers are drawn as body and kit and animated from their ghost (docs/functions/soldier-model.md, stages 1 and 2); `BundlePlayer::update` is not reversed, so there are no fades and no one-shots; the pose (crouch, prone) and the weapon's own messages are not read; no weapon mesh in the hands | a bot crouches and fires on screen, with the weapon he has out |
| the spawn screen on an idle round | with a live round it rebuilds fifteen times and fills (157 pieces) and DONE works; on a round where nothing moves it rebuilds **once**, empty, and the player cannot spawn at all. It costs more than the screen: a player with no controlled object is skipped by the server's whole relevance pass (`ServerConnection::updateGhostManager`, Linux 0x43ca90, step 5), so other players are then drawn from updates a second apart and jump ten metres between frames. `--group <n>` spawns without the screen meanwhile | the spawn screen fills on a server whose bots have not spawned yet |
...
| ping | ours and the original's differ by 5–6× on the same machine. Measured from the inside: an action is answered in 172–181 ms on average (at most 432). The server's own pace is not the cause — it sends 14 packets a second, 1.5 of its ticks apart, which is the twenty a second the connection type asks for. Where the rest of the time goes, and whether the original waits as long on the same server, is not measured | the difference is within noise, and `--group` then reports a soldier drawn interpolated |
