...
| the soldier in the air | **done**: the server's tick order (input, node, collision — `GameServer::simulateFrame` 0x45af70), the node whole with its drag (0x6f2130), the jump (0x54fb7a), steering in the air (0x54f0a5), the friction (0x6f3390) and the terrain push-out on the engine's triangles (0x6f5e30, 0x6fbc00) — `tickSoldier`. On the live server 6 of 590 states differ from our prediction by more than 1 cm, against 59 of 441 before. What is left: contacts with objects (`checkSoldierVsMesh` 0x6f44a0, ours stands in), the terrain's material map (`HeightmapPrimary.mat`, `Grass` stands in), the water half of 0x6f5e30, the spawn's first state (1.25 m off), and two states in the air one tick of gravity past ours | a run jumping around objects draws no correction |
