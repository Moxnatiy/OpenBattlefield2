...
| how a ghost is extrapolated | `SoldierNetworkable::predict` (`BF2.exe` 0x62d130) is read as far as the interpolation branch and the clock (`now − GSInterpolationTime`, the game object's +0x50). Its **extrapolation** branch is not: it works in fractions of the gap between the last two samples (`(now − newest) / gap`) and scales a vector by that and by the gap in seconds, rather than running a free line along the velocity the way we do. Ours sinks a soldier into the ground when his reported velocity points down | a soldier standing on a slope does not sink between updates |
