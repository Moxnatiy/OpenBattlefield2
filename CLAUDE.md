...
| a soldier sinking into the ground | **not** the extrapolation: `SoldierNetworkable::predict` is now read whole (Linux 0x5dc070, docs/functions/network-events.md) and its line along the velocity is the one we already draw. Some soldiers still stand 0.12–0.23 m above the ground where others stand at 1.00, so the fault is in the position or the pivot we place them by, and it is not found | every drawn soldier's "above the ground on average" is the same number |
...
| a sprinter's clip | `Soldier::updateAnimationSystems` (Linux 0x5545e0) hands a soldier who cannot fire speed 7.0 straight ahead, and `stand_sprint`'s range is 5.5–6.3; by the client's own `isWithinRange` (0x7fe720) nothing plays. Ours shows exactly that: 11% of moving frames, all at 6–7 m/s, have no movement clip and the soldier glides standing (docs/functions/animation-system.md). What the original draws is not measured | the original, watched: does a bot sprinting straight play the sprint clip |
