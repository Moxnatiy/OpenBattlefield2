...
echo "==> d3d9.dll (i686, debug)"
( cd "$SRC/windows" && cargo build --profile dev -p d3d9 --target i686-pc-windows-msvc )

echo "==> mtld3d.so (x86_64, debug)"
( cd "$SRC/unix" && cargo build --profile dev --target x86_64-apple-darwin -p mtld3d-unix )
cp "$SRC/unix/target/x86_64-apple-darwin/debug/libmtld3d_unix.dylib" \
   "$SRC/unix/target/x86_64-apple-darwin/debug/mtld3d.so"

echo "==> у пляшку"
cp "$SRC/windows/target/i686-pc-windows-msvc/debug/d3d9.dll" \
   "$B/drive_c/windows/syswow64/d3d9.dll"
cp "$SRC/unix/target/x86_64-apple-darwin/debug/mtld3d.so" \
