...
class JoinSequence {
 public:
  ...
  // Пауза між кроками рукостискання. Три секунди — наш обережний запас
  // із перших дослідів, а не вимога сервера: у знятому трафіку оригінал
  // шле `NELoadComplete` і перевірку вмісту **одним пакетом**, а
  // `NEDatabaseComplete` — через 1.1 с.
  static constexpr std::chrono::seconds kStepDelay{3};
