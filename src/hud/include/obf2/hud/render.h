...
struct Context {
  ...
  // Розмір значка точки. **Не виміряний.** Самих значків у знімку кадру
  // немає взагалі — ні окремими викликами, ні партією: гра, схоже, малює
  // їх у проміжну текстуру карти. У бінарі команди розміру теж немає
  // (є лише setMapIconAlphaTransparency, обробник 0x415ee0), а функція
  // підготовки значків 0x74fb70 сталих не містить. Тому тут стоїть
  // власний розмір текстури miniMap_CP.tga — 33x33, — а не підігнане
  // число: коли знайдеться код малювання, замінимо на справжнє.
  float mapMarkerSize = 33.0f;
};
