...
struct Context {
  ...
  float spawnMarkerSize = 11.0f;
  // Розмір світу рівня в метрах — ним переводимо координати позначок
  // у частки картинки.
  float mapWorldSize = 2048.0f;
  // Розмір значка в базових 800x600. Виміряний непрямо, бо самі значки
  // гра окремими викликами не малює. Зате малює підписи: у знімку кадру
  // це партія на 306 вершин, тобто 51 літера — рівно стільки, скільки в
  // назвах чотирьох точок Dalian_plant без пробілів. Її габарит —
  // 361.5, 278.2 розміром 246.0x123.8.
  //
  // Підпис стоїть під значком, тож зсув від центра точки дорівнює
  // половині значка плюс верхній відступ шрифту. У нас при значку 16
  // виходило +10 при габариті 362.2, 280.7 245.0x123.6 — тобто ширина
  // й висота вже збігалися, а верх стояв на 2.5 нижче. Звідси
  // половина значка 5.5, а сам він — 11.
  float mapMarkerSize = 11.0f;
};
