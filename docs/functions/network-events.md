...
### Причина: діри в нумерації пакетів

Пішли по ланцюжку вимірюванням. Пакети проходять перевірку заголовка в
`NetServer::handleDataPacket`, `confirmPacket` повертає успіх, вони
потрапляють у чергу `addPacketToRecvQueue` — а `getClient` для них
викликається лише п'ять разів. Тобто черга наповнюється, але сервер
перестає її вибирати.

Друк номерів показав чому:

```
шлю подію: наш seq=7
шлю подію: наш seq=8
шлю подію: наш seq=10   <- дірка, 9 з'їла відповідь на пінг
шлю подію: наш seq=11
шлю подію: наш seq=13
```

Шестибітний лічильник рахує **лише пакети даних**. Ми ж витрачали його
ще й на відповіді на пінги, і в потоці даних з'являлися пропуски. Сервер
чекає на пропущений номер і спиняється зовсім — без від'єднання, просто
тиша в обидва боки.

Перевірка навпростець: якщо на пінги не відповідати взагалі, з 11
пакетів даних сервер обробляє 10. Якщо відповідати, не витрачаючи
номер, — 15 із 16, і сервер одразу вивалює весь світ.

У зонді це виправлено. У клієнті на C++ та сама зміна дала зворотний
ефект (лишився один пакет замість дванадцяти), тож там поки лишається
як було — розібратися, чому саме, ще належить.
