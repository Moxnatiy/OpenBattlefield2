...
З обходом сервер доходить до `clientSendDatabaseComplete` — і йде
гілкою **відмови 27**. Позначка `contentValid` не виставлена.

Простежили точно, де вона губиться. Після збігу хешів код робить так:

```
462ac0:  ebx = 1                          (успіх)
462ae9:  playerManager->[0x40](номер)
462b02:  gameServer->[0x2b0](номер)
462b08:  якщо результат == 0 -> пропустити
462b1f:  client->[0x58](1)                setContentValid(true)
```

Точка зупину показала: до `462ac0` доходимо, а до `462b1f` — ні. Тобто
`gameServer->[0x2b0]` повертає порожнє посилання, і виклик, який ставить
позначку, просто пропускається. Що це за метод і чому він нічого не
знаходить — наступне питання.

### Про порядок подій в одному пакеті

Мережеві події (`PostRemoteEvent`) не виконуються одразу: вони йдуть
через чергу подій і спрацьовують наступним тактом. Перевірка вмісту,
навпаки, виконується на місці. Тому «покласти все в один пакет» —
не те саме, що надіслати послідовно: у спільному пакеті перевірка
відпрацює **раніше** за `NELoadComplete`, а їй потрібен стан більший за
одиницю.

Робочий розклад на чотири пакети: виклик, `ClientInfo`, `NELoadComplete`
окремо, а вже потім перевірка вмісту разом із рештою.

Привиди сервер пише — 65 разів за сеанс, — але не нам: у нього є власне
локальне з'єднання, і всі ті записи йдуть туди. Нам не приходить жодного,
бо ми не проходимо `isClientReady`.