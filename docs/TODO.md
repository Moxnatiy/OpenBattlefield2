...
- ~~Немає зіткнень~~ — частково: солдат тримається землі (білінійна вибірка
  з карти висот) і падає під гравітацією. Геометрія об'єктів у зіткненнях
  ще не бере участі — `.collisionmesh` не читається.
