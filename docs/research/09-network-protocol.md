...
Відповісти на виклик (`GameServer::challengeResponse`) і надіслати
`ClientInfo` — після цього сервер починає слати стан світу через
`GhostManager`. Ці 17 бітів каркасу теж треба доробити.
