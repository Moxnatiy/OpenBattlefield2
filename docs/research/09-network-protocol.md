...
Надіслати `ClientInfo` (`GameServer::handleClientInfo`) — після цього
сервер має завести гравця й почати слати стан світу через `GhostManager`.
Поки `admin.listPlayers` порожній, бо ми ще не представилися.
